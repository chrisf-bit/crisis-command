...
import { RESOURCE_CHANNELS, CHANNEL_META, CHANNEL_KPI_MAP } from '../types'
...
// Tick marks — 26 ticks (values 0,2,4,...,50), major every 5th
const TICKS = Array.from({ length: 26 }, (_, i) => ({
  angle: START_ANGLE + ((i * 2) / 50) * TOTAL_SWEEP,
  isMajor: i % 5 === 0,
}))
...
function ArcGauge({
  ...
  const valueAngle = START_ANGLE + (value / 50) * TOTAL_SWEEP
  ...
  const angleFromPointer = useCallback(
    (e: React.PointerEvent) => {
      ...
      onAllocate(channel, Math.round(Math.max(0, Math.min(50, pct * 50))))
    },
...
export default function ResourceAllocator({
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full w-full px-4 py-3 gap-3 relative z-[1]">
      {/* CTA instruction */}
      <div className="flex items-center gap-3">
        <div className="w-10 h-px" style={{ background: 'linear-gradient(90deg, transparent, rgba(0,229,255,0.3))' }} />
        <span
        ...
        >
          {locked ? '● ALLOCATION LOCKED' : 'DRAG DIALS TO ALLOCATE RESOURCES'}
        </span>
        <div className="w-10 h-px" style={{ background: 'linear-gradient(270deg, transparent, rgba(0,229,255,0.3))' }} />
      </div>
      ...
      {!locked && (
        <span
          ...
          style={{ color: 'rgba(224,230,240,0.3)' }}
        >
          Higher allocation amplifies impact on the linked metric
        </span>
      )}
    </div>
  )
